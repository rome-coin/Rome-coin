...
// Copyright (c) 2014 rome-coin Developers
...
uint256 hashGenesisBlock("0x18e678ae466293df31d8a72996474fefad4d6e0a695fd7d8224122e27f47315e");
//uint256 hashGenesisBlock("0x29bee4fc00c6fb655c92394a589afbf3b1bda9b4be952f791288d864da238fa4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Rome-coin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Rome-coin Signed Message:\n";
...
{
    // Rome-coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
//static const int64 nTargetTimespan = 3*60*60;//3 * 60 * 60; // Rome-coin:every 3 hours
//static const int64 nTargetSpacing =  3*60; // Rome-coin:3 minutes
//static const int64 nInterval = nTargetTimespan / nTargetSpacing;

static int64 nTargetTimespan = 2*60*60;//2 * 60 * 60; // Rome-coin:every 2 hours
static int64 nTargetSpacing =  30; // Rome-coin:1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Rome-coin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // Rome-coin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "start rmc 20140404";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1396604495;//2014-4-4 17:41:35 //1391830000;//2014-2-8 11:26:40
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 9878244;//

        if (fTestNet)
        {
            block.nTime    = 1394360279;
            block.nNonce   = 0;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x58d071623525e5a2c30e853e1a368c49263f4d52e433855099d490816eb43bfd"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Rome-coin: increase each by adding 2 to bitcoin's value.
...
void static RomecoinMiner(CWallet *pwallet)
{
    printf("RomecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rome-coin-miner");
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RomecoinMiner, pwallet));
}
